...
const App: React.FC = () => {
  ...
  const uniqueValues = useMemo(() => {
    const chaines = Array.from(new Set(interventions.map((i: Intervention) => i.chaine).filter(Boolean))) as string[];
    const equipements = Array.from(new Set(interventions.map((i: Intervention) => i.equipement).filter(Boolean))) as string[];
    const mecaniciens = Array.from(new Set(interventions.map((i: Intervention) => i.mecanicien).filter(Boolean))) as string[];
    const types = Array.from(new Set(interventions.map((i: Intervention) => i.type).filter(Boolean))) as string[];
    const nature_pannes = Array.from(new Set(interventions.map((i: Intervention) => i.nature_panne).filter(Boolean))) as string[];
  ...
  const filteredInterventions = useMemo(() => {
    return interventions.filter((interv: Intervention) => {
